...
class Desc2MOF(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.alpha = config.get(
            "alpha", 0
        )  # rmsd loss using locator btw topology and node
        ...
        self.cn_end = (
            self.cn_start + self.cn_size
        )  # CN vocab size (e.g: [CN_2]...[CN_8] etc)

        self.edge_cn_ids = torch.tensor(
            [
                MOF_ENCODE_DICT["[CN_2]"],
            ],
            dtype=torch.long,
        )
        self.node_cn_ids = [
            MOF_ENCODE_DICT[f"[CN_{k}]"] for k in [3, 4, 5, 6, 7, 8, 9, 10, 12, 24]
        ]
    ...
    def on_validation_epoch_end(self):
        ...
        totals = {
            k: sum(x[k] for x in self.validation_outputs)
            for k in [
                "val_loss",
                "batch_size",
    ...
    def test_step(self, batch, batch_idx):
        ...
        topo_mask = (
            (target_output >= self.topo_start) & (target_output < self.topo_end) & mask
        )
        node_mask = (
            (target_output >= self.node_start) & (target_output < self.node_end) & mask
        )
        edge_mask = (
            (target_output >= self.edge_start) & (target_output < self.edge_end) & mask
        )
        selfies_mask = (
            (target_output >= self.selfies_start)
            & (target_output < self.selfies_end)
            & mask
        )
        cn_mask = (
            (target_output >= self.cn_start) & (target_output < self.cn_end) & mask
        )
        ...
        all_correct_top1 = (
            correct_pad_top1.all(dim=1).float().sum().item()
        )  # [B] -> scalar mean
        # block-level (node/edge segment all-correct)
        node_all_correct_top1, node_all_total_top1 = (
            self._block_all_correct_total_from_preds(
                preds_top1, target_output, self.node_cn_ids
            )
        )
        edge_all_correct_top1, edge_all_total_top1 = (
            self._block_all_correct_total_from_preds(
        ...
        node_all_correct_top5, node_all_total_top5 = (
            self._block_all_correct_total_from_correct_matrix(
                correct_top5, target_output, self.node_cn_ids
            )
        )
        edge_all_correct_top5, edge_all_total_top5 = (
            self._block_all_correct_total_from_correct_matrix(
        ...
        preds = self.generate(
            desc,
        )  # [B, T] #max_token_len=target_output.size(1)
        correct = (preds == target_output) & mask  # [B, T]
    ...
    def on_test_epoch_end(self):
        ...
        keys = [
            ...
            "cn_total",
            "node_all_total",
            "edge_all_total",  # block-level denom
            # autoregressive (generate)
            "token_correct",
            "all_correct",
            "topo_correct",
            ...
            "node_correct_top1",
            "edge_correct_top1",
            "selfies_correct_top1",
            "cn_correct_top1",
            "node_all_correct_top1",
            "edge_all_correct_top1",
            # top5
            "token_correct_top5",
            "all_correct_top5",
            "topo_correct_top5",
            "node_correct_top5",
            "edge_correct_top5",
            "selfies_correct_top5",
            "cn_correct_top5",
            "node_all_correct_top5",
            "edge_all_correct_top5",
        ]
        ...
        # ---- averages / rates ----
        avg_loss = (
            totals["test_loss"] / totals["batch_size"]
        ...
        avg_node_all_acc = safe_div(
            totals["node_all_correct"], totals["node_all_total"]
        )
        avg_edge_all_acc = safe_div(
            totals["edge_all_correct"], totals["edge_all_total"]
        )

        # top1
        avg_token_acc_top1 = safe_div(
            totals["token_correct_top1"], totals["token_total"]
        )
        avg_all_correct_top1 = safe_div(
            totals["all_correct_top1"], totals["batch_size"]
        )
        ...
        avg_selfies_acc_top1 = safe_div(
            totals["selfies_correct_top1"], totals["selfies_total"]
        )
        avg_cn_acc_top1 = safe_div(totals["cn_correct_top1"], totals["cn_total"])
        avg_node_all_acc_top1 = safe_div(
            totals["node_all_correct_top1"], totals["node_all_total"]
        )
        avg_edge_all_acc_top1 = safe_div(
            totals["edge_all_correct_top1"], totals["edge_all_total"]
        )

        # top5
        avg_token_acc_top5 = safe_div(
            totals["token_correct_top5"], totals["token_total"]
        )
        avg_all_correct_top5 = safe_div(
            totals["all_correct_top5"], totals["batch_size"]
        )
        ...
        avg_selfies_acc_top5 = safe_div(
            totals["selfies_correct_top5"], totals["selfies_total"]
        )
        avg_cn_acc_top5 = safe_div(totals["cn_correct_top5"], totals["cn_total"])
        avg_node_all_acc_top5 = safe_div(
            totals["node_all_correct_top5"], totals["node_all_total"]
        )
        avg_edge_all_acc_top5 = safe_div(
            totals["edge_all_correct_top5"], totals["edge_all_total"]
        )

        # ---- save predictions (optional) ----
        all_pred_tokens = torch.concat(
            [x["pred_tokens"] for x in self.test_outputs], dim=0
        )
        mof_name_pred = ["+".join(token) for token in decode_token2mof(all_pred_tokens)]
        if self.config.get("save_test", False):
            exp_name = self.config["exp_name"]
            pd.DataFrame(mof_name_pred, columns=["filename"]).to_csv(
                f"{exp_name}_mof_name_pred.csv", index=None
            )
        ...
        self.log(
            "test/avg_selfies_acc", avg_selfies_acc, prog_bar=False, sync_dist=True
        )
        self.log("test/avg_cn_acc", avg_cn_acc, prog_bar=False, sync_dist=True)
        self.log(
            "test/avg_node_all_acc", avg_node_all_acc, prog_bar=False, sync_dist=True
        )
        self.log(
            "test/avg_edge_all_acc", avg_edge_all_acc, prog_bar=False, sync_dist=True
        )

        # top1
        self.log(
            "test/avg_token_acc_top1", avg_token_acc_top1, prog_bar=True, sync_dist=True
        )
        self.log(
            "test/avg_all_correct_top1",
            avg_all_correct_top1,
            prog_bar=True,
            sync_dist=True,
        )
        self.log(
            "test/avg_topo_acc_top1", avg_topo_acc_top1, prog_bar=False, sync_dist=True
        )
        self.log(
            "test/avg_node_acc_top1", avg_node_acc_top1, prog_bar=False, sync_dist=True
        )
        self.log(
            "test/avg_edge_acc_top1", avg_edge_acc_top1, prog_bar=False, sync_dist=True
        )
        self.log(
            "test/avg_selfies_acc_top1",
            avg_selfies_acc_top1,
            prog_bar=False,
            sync_dist=True,
        )
        self.log(
            "test/avg_cn_acc_top1", avg_cn_acc_top1, prog_bar=False, sync_dist=True
        )
        self.log(
            "test/avg_node_all_acc_top1",
            avg_node_all_acc_top1,
            prog_bar=False,
            sync_dist=True,
        )
        self.log(
            "test/avg_edge_all_acc_top1",
            avg_edge_all_acc_top1,
            prog_bar=False,
            sync_dist=True,
        )
        ...
        # top5
        self.log(
            "test/avg_token_acc_top5", avg_token_acc_top5, prog_bar=True, sync_dist=True
        )
        self.log(
            "test/avg_all_correct_top5",
            avg_all_correct_top5,
            prog_bar=True,
            sync_dist=True,
        )
        self.log(
            "test/avg_topo_acc_top5", avg_topo_acc_top5, prog_bar=False, sync_dist=True
        )
        self.log(
            "test/avg_node_acc_top5", avg_node_acc_top5, prog_bar=False, sync_dist=True
        )
        self.log(
            "test/avg_edge_acc_top5", avg_edge_acc_top5, prog_bar=False, sync_dist=True
        )
        self.log(
            "test/avg_selfies_acc_top5",
            avg_selfies_acc_top5,
            prog_bar=False,
            sync_dist=True,
        )
        self.log(
            "test/avg_cn_acc_top5", avg_cn_acc_top5, prog_bar=False, sync_dist=True
        )
        self.log(
            "test/avg_node_all_acc_top5",
            avg_node_all_acc_top5,
            prog_bar=False,
            sync_dist=True,
        )
        self.log(
            "test/avg_edge_all_acc_top5",
            avg_edge_all_acc_top5,
            prog_bar=False,
            sync_dist=True,
        )
    ...
    def predict_step(self, batch, batch_idx):
        ...
        self.pred_outputs.append(
            {"desc": desc_origin.detach().cpu(), "pred": pred.detach().cpu()}
        )

    def on_predict_epoch_end(self):
        # 1. Gather data from current GPU into tensors
        local_desc = torch.cat([out["desc"] for out in self.pred_outputs], dim=0).to(
            self.device
        )
        local_pred = torch.cat([out["pred"] for out in self.pred_outputs], dim=0).to(
            self.device
        )

        # 2. Gather data from all GPUs (all_gather)
        # Returns list [Rank0_tensor, Rank1_tensor, ...]
        all_desc_tensors = self.all_gather(local_desc)
        all_pred_tensors = self.all_gather(local_pred)

        # 3. Only save file on main process (Rank 0)
        if self.trainer.is_global_zero:
            # all_gather results are [Rank, Batch, ...], concatenate along batch dim
            ...
            feature_name_dir = self.config.get(
                "feature_name_dir",
                f"{__desc2mof_dir__}/../desc2mof/data/feature_name.txt",
            )
    ...
    def _block_all_correct_total_from_preds(
        ...
        is_cn = (tgt_ids.unsqueeze(-1) == cn_ids.view(1, 1, -1)).any(
            dim=-1
        ) & valid_pos  # [B,T]
        ...
        starts, _ = torch.topk(
            start_candidates, k=Kmax, largest=False, dim=1
        )  # [B,Kmax]
        ...
        # after_start & is_sep
        after_start = idx.unsqueeze(1) > starts.unsqueeze(
            -1
        )  # [B,K,T] idx[B,1,512] starts[B,2,1] -> [B,2,512]
        ...
        block_mask = (pos >= starts.unsqueeze(-1)) & (
            pos <= ends.unsqueeze(-1)
        )  # [B,K,T]
    ...
    def _edge_all_correct_total_from_preds(self, pred_ids, tgt_ids):
        ...
        for b in range(B):
            ...
            edge_all_total += 1.0
            edge1_ok = (
                (pred[sep1 + 1 : sep2] == tgt[sep1 + 1 : sep2]).all().item()
                if (sep2 > sep1 + 1)
                else True
            )
            if edge1_ok:
...
class EmbeddingWithPositionalEncoding(nn.Module):
    def __init__(self, hid_dim, dropout=0.1, max_len=512, batch_first=True):
        ...
        self.dropout = (
            nn.Dropout(p=dropout) if hid_dim > 1 else None
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.pe is not None:
            ...
            assert seq_len <= self.pe.shape[0], (
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.pos_encoder = EmbeddingWithPositionalEncoding(
            hid_dim, max_len=max_len
        )  # (B, L ,hid_dim)

        self.token_embedding = TokenEmbedding(total_vocab_size, hid_dim)
        self.pos_decoder = PositionalEncodingOnly(hid_dim)
        self.decoder = TransformerDecoder(
            hid_dim, num_layers=decoder_layers, num_heads=num_heads
        )
        ...
        self.cn_end = (
            self.cn_start + self.cn_size
        )  # CN vocab size (e.g: [M_CN_2]...[O_CN_8] etc)

        self.id2token = {v: k for k, v in self.encode_dict.items()}  # id -> string
        self.topo_ids = torch.arange(
            self.topo_start, self.topo_end
        )  # vocab 상 topo id range
        self.selfies_ids = torch.arange(
            self.selfies_start, self.selfies_end
        )  # selfies range
    ...
    def forward(self, desc, target_input):
        ...
        tgt_mask = torch.triu(
            torch.ones(T, T, device=desc.device, dtype=torch.bool), diagonal=1
        )
    ...
    def generate(self, desc, pad_token, eos_token, max_token_len=512, temperature=1):
        ...
        if cur_len < max_token_len:
            ...
            pad_block = torch.full(
                (B, pad_len), pad_token, dtype=torch.long, device=device
            )
    ...
    def generate_with_hidden(
        self, desc, pad_token, eos_token, max_token_len=512, temperature=1
    ):
        """
        ...
        for _ in range(max_token_len):
            ...
            all_decoder_outputs.append(
                decoder_output[:, -1:, :]
            )  # Save the hidden state of the final token only
    ...
    def generate_beam(
        ...
        def next_logits(seq_flat, mem_flat, mem_pad_mask_flat):
            ...
            pos_emb = self.pos_decoder.pe[:, :Tcur, :].to(
            ...
            last_out = out[
                torch.arange(seq_flat.size(0), device=device), last_valid_idx
            ]
        ...
        def beam_step(seqs, scores_raw, lengths, mem_use, mem_pad_mask):
            ...
            topk_select, topk_idx = torch.topk(
                cand_scores_select_flat, beam_width, dim=-1
            )
            ...
            # prefix 선택
            new_seqs = seqs.gather(
                1, beam_idx.unsqueeze(-1).expand(-1, -1, Tcur)
            )  # (B,beam_width,Tcur)
            new_seqs = torch.cat(
                [new_seqs, tok_idx.unsqueeze(-1)], dim=-1
            )  # (B,beam_width,Tcur+1)
        ...
        # 첫 스텝
        seqs, scores_raw, lengths = beam_step(
            seqs, scores_raw, lengths, memory, memory_key_padding_mask
        )
    ...
    def generate_beam_session_greedy(
        ...
        def last_valid_token(seq: torch.Tensor, pad_token: int):
            ...
            last_idx = (
                torch.arange(seq.size(1), device=seq.device) * valid_mask
            ).argmax(dim=1)
        ...
        # ============================================================
        def next_logits(
            seq_flat: torch.Tensor, mem_flat: torch.Tensor, pos_flat: torch.Tensor
        ):
            ...
            Tcur = seq_flat.size(1)
            tgt_mask = torch.triu(
                torch.ones(Tcur, Tcur, device=device, dtype=torch.bool), diagonal=1
            )
            ...
            last_valid_idx = (torch.arange(Tcur, device=device) * valid_mask).argmax(
                dim=1
            )
        ...
        def beam_expand(seqs, scores, pos_ids, k, mem_use):
            ...
            real_last_tokens = seq_flat[
                torch.arange(flatN, device=device), last_valid_idx
            ]
        ...
        def greedy_until_sep(seqs, scores, pos_ids, mem_sel, max_steps):
            for _ in range(max_steps):
                ...
                mem_flat = (
                    mem_sel.unsqueeze(1)
                    .expand(B2, beam2, mem_sel.size(1), mem_sel.size(2))
                ...
                is_done = (
                    (last_tok == self.sep_token)
                    | (last_tok == self.eos_token)
                    | (last_tok == self.pad_token)
                )
                if is_done.any():
                    logits[is_done, :] = -float("inf")
                    logits[is_done, self.pad_token] = 0.0
                ...
                is_finished = (
                    (next_tok == self.sep_token)
                    | (next_tok == self.eos_token)
                    | (next_tok == self.pad_token)
                )
                if is_finished.all():
                    break
        ...
        # Topology
        seqs, scores, pos_ids = beam_expand(
            seqs, scores, pos_ids, k=beam_width, mem_use=memory
        )
